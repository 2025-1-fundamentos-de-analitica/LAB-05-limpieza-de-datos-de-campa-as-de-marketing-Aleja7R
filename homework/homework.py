...
def clean_campaign_data():
    ...
    input_path_dir = "files/input/"
    output_path_dir = "files/output/"   
    ...
    # Leer de los .zip 
    for archivo in os.listdir(input_path_dir):
        if archivo.endswith(".zip"):
            zip_path = os.path.join(input_path_dir, archivo)
    ...
    # Verificar quelos nombres de columnas no tengan espacios o símbolos extraños
    df_total.columns = [col.strip().lower().replace(" ", "_") for col in df_total.columns]

    # En caso de que si no existe, asignar client_id 
    ...
    df_client.to_csv(os.path.join(output_path_dir, "client.csv"), index=False)
    ...
    df_campaign.to_csv(os.path.join(output_path_dir, "campaign.csv"), index=False)
    ...
    df_economics.to_csv(os.path.join(output_path_dir, "economics.csv"), index=False)
